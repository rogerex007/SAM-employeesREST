...
```bash
microservices$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
microservices$ sam local start-api
microservices$ curl http://localhost:3000/
```
...
```yaml
      Events:
        ...
        ReadEmployee:
          Type: Api
          Properties:
            Path: /Employees/getEmployee/{id} #Write a employeed id
            Method: get
        ReadEmployees:
          Type: Api
          Properties:
...
```bash
microservices$ sam logs -n HelloWorldFunction --stack-name microservices --tail
```
